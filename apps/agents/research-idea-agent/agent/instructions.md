...
1. **Nederlands** — Alle menselijke tekstvelden (`ideaTitle`, `titleProposals`, `whyInteresting`, `claim`, `evidence`, `whyInterview`, `interviewerNotes`, `questions`, `researchSummary`, namen in prose) schrijf je in het **Nederlands**. JSON-sleutels blijven Engels.
...
- Interviewvelden: `neonPersonId`, `fullName`, `contactType`, optioneel `contactTypeDetail`, `neonClubId`, `clubName`, optioneel `neonTeamId`/`teamName`, `whyInterview`, `interviewerNotes` (briefing: wie / waarom / doel), `questions` (1–8 Nederlandse interviewvragen)
