...
| App `users.role` | from env `AGENT_ACCESS_ROLE` (**default `admin`** — full redactie access) |
| GitHub account | **none** |
...
- Default: **`admin`** — full access to custom admin (overview, later nieuwsbrieven/abonnees/instellingen, and the Better Auth gate for `/keystatic` when that lands).
- Override via `AGENT_ACCESS_ROLE=admin|journalist|viewer` only if you intentionally want a weaker agent (e.g. viewer-only smoke tests).
- Product decision: Cursor agents are trusted operators of this repo; least-privilege for agents is deferred until multi-agent/multi-tenant needs appear.
...
- JSON body: `{ "ok": true, "email": "cursor-agent@agents.devoetbalgazet.local", "role": "admin" }`.
...
| `AGENT_ACCESS_ROLE` | Next.js/Convex | `admin` \| `journalist` \| `viewer` (**default `admin`**) |
| `AGENT_ACCESS_EMAIL` | Optional override | Default `cursor-agent@agents.devoetbalgazet.local` |

Also list placeholders in `.env.example` with comments: “Cursor agents only; leave unset in production.” **Never commit real secret values.**

### Where to store the generated secrets

Secrets are generated once and stored **outside git**:

| Location | What to put |
|----------|-------------|
| Cursor Dashboard → Cloud Agents → Secrets (this environment) | `AGENT_ACCESS_ENABLED=true`, `AGENT_ACCESS_SECRET` (**Runtime Secret**), `AGENT_USER_PASSWORD` (**Runtime Secret**), `AGENT_ACCESS_ROLE=admin` |
| Local / agent VM gitignored file `.env.agent-access.local` | Same keys; already gitignored via `.env.*` |
| Production Vercel | Leave **unset** by default |

Do not paste secrets into plan markdown, commits, PR bodies, or `AGENTS.md`.
...
3. `AGENT_USER_PASSWORD=<long random>` as **Runtime Secret**
4. `AGENT_ACCESS_ROLE=admin`
5. Same values available to the Next.js process the agent starts (merge into process env or `apps/web/.env.local`)
...
2. Set AGENT_ACCESS_* secrets (role defaults to admin).
3. Start web + convex.
4. Open /admin/agent-inloggen, paste AGENT_ACCESS_SECRET, submit.
5. You should land on /admin as admin.
```
